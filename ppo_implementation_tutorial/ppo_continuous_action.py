...
def make_env(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = gym.wrappers.ClipAction(
            env
        )  # unclipped action still saved in the storage for the forward pass consistency during training, the environment will execute the clipped actions
        env = gym.wrappers.NormalizeObservation(
            env
        )  # observation normalization (running mean)
...
if __name__ == "__main__":
    ...
    ), "only continuous action space is support"
    print(
